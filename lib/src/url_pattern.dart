...
class UrlPattern implements Pattern {
  ...
  String reverse(Iterable args, {bool useFragment = false}) {
      ...
      if (c == '\\' && escaped == false) {
        ...
        if (c == '(') {
          ...
          if (escaped && depth == 0) {
            ...
            if (depth == 0) throw ArgumentError('unmatched parentheses');
            depth--;
            if (depth == 0) {
              // append the nth arg
              if (argsIter.moveNext()) {
                ...
                throw ArgumentError('more groups than args');
              }
            }
          }
        } else if (depth == 0) {
    ...
    if (depth > 0) {
      throw ArgumentError('unclosed group');
    }
    return sb.toString();
  }
  ...
  List<String> parse(String path) {
    ...
    if (match == null) {
      throw ArgumentError('no match for $path');
    }
    var result = <String>[];
    for (int i = 1; i <= match.groupCount; i++) {
      result.add(match[i] as String);
    }
    return result;
  }
  ...
  Match matchAsPrefix(String string, [int start = 0]) =>
      regex.matchAsPrefix(string, start) as Match;
  ...
  _parse(String pattern) {
    ...
    for (var i = 0; i < chars.length; i++) {
      ...
      if (depth == 0) {
        ...
        if (c == r'\') {
          ...
          if (_specialChars.hasMatch(c)) {
            ...
            if (escaped) {
              ...
              sb.write('(');
              if (lastGroupEnd == i - 1) {
                throw ArgumentError('ambiguous adjecent top-level groups');
              }
              depth = 1;
            }
          } else if (c == ')') {
            if (escaped) {
              ...
              throw ArgumentError('unmatched parenthesis');
            }
          } else if (c == '#') {
        ...
        if (c == '(' && !escaped) {
          ...
          depth--;
          if (depth < 0) throw ArgumentError('unmatched parenthesis');
          if (depth == 0) {
            lastGroupEnd = i;
          }
        } else if (c == '#') {
          // TODO(justinfagnani): what else should be banned in groups? '/'?
          throw ArgumentError('illegal # inside group');
        }
        escaped = (c == r'\' && !escaped);
        sb.write(c);
      }
    ...
    sb.write(r'$');
    _regex = RegExp(sb.toString());
  }

  _setBasePattern(String basePattern) {
    if (_hasFragment == true) {
      throw ArgumentError('multiple # characters');
    }
    _hasFragment = true;
    _baseRegex = RegExp('$basePattern\$');
  }
}